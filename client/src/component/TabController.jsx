...
const TabController = (props) => {
  ...
  useEffect(() => {
    const e = document.getElementById('tab0');
    e.checked = true;
    setUnderlineWidth(e.labels[0].offsetWidth);
    setunderlineXpos(e.labels[0].offsetLeft);
  }, []);
    ...
    <>
      <div className='tab-container'>
        <div className='tab-header mb15'>
          {menuArr.map((data, index) => (
            <label className='tab' key={'tab' + index}>
              <input
                ...
                className='tab'
                id={'tab' + index}
                onClick={(e) => fnTabOnChange(index, e)}
                value={'item' + index}
                name='group'
              />
              {data.menu}
          ...
          <div
            className='underline'
            style={{ width: underlineWidth + 'px', left: underlineXpos }}
            id='underline'
          />
        </div>
        <div></div>
